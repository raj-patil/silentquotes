...
public class ProfileInteractor {
    ...
    {
            DatabaseReference databaseReference =databaseHelper.getDatabaseReference().child(DatabaseHelper.BRODCAST_DB_KEY).child(id);
            databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    Brodcast brodcast = dataSnapshot.getValue(Brodcast.class);
                    listener.onObjectChanged(brodcast);
                }
    ...
    private Query getConnectionSearchQuery(DatabaseReference databaseReference, String childOrderBy, String searchText) {
    ...
    public ValueEventListener searchConnections(String searchText, OnDataChangedListener<Connection> onDataChangedListener) {
        DatabaseReference reference = databaseHelper.getDatabaseReference().child(DatabaseHelper.CONNECTION_DB_KEY).child(FirebaseAuth.getInstance().getCurrentUser().getUid());
        ValueEventListener valueEventListener = getConnectionSearchQuery(reference, "username", searchText.toLowerCase()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                List<Connection> list = new ArrayList<>();
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    Connection profile = snapshot.getValue(Connection.class);

                    list.add(profile);
                    LogUtil.logDebug(TAG, "search profile: " + profile.getId());
                }
    ...
    public ValueEventListener searchRequestConnection(String searchText, OnDataChangedListener<Connection> onDataChangedListener) {
        DatabaseReference reference = databaseHelper.getDatabaseReference().child(DatabaseHelper.REQUEST_DB_KEY).child(FirebaseAuth.getInstance().getCurrentUser().getUid());
        ValueEventListener valueEventListener = getSearchQuery(reference, "id", searchText).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                List<Connection> list = new ArrayList<>();
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
