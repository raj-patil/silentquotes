...
public class FollowInteractor {
    ...
    public void getBookMarkPost(String userId, OnDataChangedListener<BookMark> listner) {
        databaseHelper
                .getDatabaseReference()
                .child(DatabaseHelper.POST_BOOKMARK_DB_KEY)
                .child(userId).orderByChild("createdDate")
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        List<BookMark> list = new ArrayList<>();
                        for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                            BookMark followingPost = snapshot.getValue(BookMark.class);
                            list.add(followingPost);
                        }
                        Collections.reverse(list);
                        listner.onListChanged(list);
                    }

                    @Override
                    public void onCancelled(DatabaseError databaseError) {
                        LogUtil.logDebug(TAG, "getBookMarkPost, onCancelled");
                    }
                });
