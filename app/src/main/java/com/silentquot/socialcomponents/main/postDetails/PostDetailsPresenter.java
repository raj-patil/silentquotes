...
import com.silentquot.socialcomponents.managers.CommentManager;
...
class PostDetailsPresenter extends BasePresenter<PostDetailsView> {
            ...
            public void onObjectChanged(Post obj) {
                ifViewAttached(view -> {
                    if (obj != null) {
                        ...
                        view.initBookMarkController(post);
                        fillInUI(post);
                        view.updateCounters(post);
                        initLikeButtonState();
                        initBookMarkButtonState();
    ...
    private void initBookMarkButtonState() {
        FirebaseUser firebaseUser = FirebaseAuth.getInstance().getCurrentUser();
        if (firebaseUser != null && post != null) {
            postManager.hasCurrentUserBookMArk(context, post.getId(), firebaseUser.getUid(),
                    exist -> {
                        ifViewAttached(view -> {
                            view.initBookMarkButtonState(exist);
                        });
                    });
        }
    }
    ...
    private void initLikeButtonState() {
        FirebaseUser firebaseUser = FirebaseAuth.getInstance().getCurrentUser();
        if (firebaseUser != null && post != null) {
            postManager.hasCurrentUserLike(context, post.getId(), firebaseUser.getUid(),
                    exist -> {
                ifViewAttached(view -> {
...
if (post.getPostType().equals("COLLAB"))
{
    view.loadCollabText(post.getCollabPostId());
        ...
        public void onObjectChanged(Profile profile) {
            ifViewAttached(view -> {
                if (profile.getPhotoUrl() != null) {
                    view.loadTextPost(profile.getPhotoUrl());
                }
