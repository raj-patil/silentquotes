...
class ProfilePresenter extends BasePresenter<ProfileView> {
    ...
    private void connectUser(String targetUserId)
    {
        ifViewAttached(BaseView::showProgress);
        connecteManager.connectUser(activity, getCurrentUserId(), targetUserId, success -> {
            ifViewAttached(view -> {
                if (success) {
                    view.setConnectStateChangeResultOk();
                    checkConnectState(targetUserId);
                } else {
                    LogUtil.logDebug(TAG, "ConnectUser , success: " + false);
                }
            });
        });
    }

    private  void acceptUser(String targetUserId)
    {
        ifViewAttached(BaseView::showProgress);
        profileManager.getProfileSingleValue(getCurrentUserId(), new OnObjectChangedListener<Profile>() {
            @Override
            public void onObjectChanged(Profile obj) {

                profileManager.getProfileSingleValue(targetUserId, new OnObjectChangedListener<Profile>() {
                    @Override
                    public void onObjectChanged(Profile obj1) {
                        connecteManager.acceptUser(activity,obj.getUsername(),obj1.getUsername(),  getCurrentUserId(), targetUserId, success -> {
                            ifViewAttached(view -> {
                                if (success) {
                                    view.setConnectStateChangeResultOk();
                                    checkConnectState(targetUserId);
    ...
    public void onCancelConnectionButtonClick(int state, String userID) {

        if (checkInternetConnection() && checkAuthorization()) {
            if (state == ConnectButton.ACCEPT_STATE) {
                cancelUser(userID);
            }
        }
    }

    private void cancelUser(String userID) {
        ifViewAttached(BaseView::showProgress);
        connecteManager.cancelUser(activity, getCurrentUserId(), userID, success -> {
            ifViewAttached(view -> {
                if (success) {
                    view.setConnectStateChangeResultOk();
                    checkConnectState(userID);
                } else {
                    LogUtil.logDebug(TAG, "followUser, success: " + false);
                }
            });
    ...
    public void onConnectButtonClick(int state , String targetUserId)
    {
        if (checkInternetConnection() && checkAuthorization()) {
            if (state == ConnectButton.CONNECT_STATE) {
                connectUser(targetUserId);
            }
            else if ( state == ConnectButton.ACCEPT_STATE)
            {
                acceptUser(targetUserId);
            }
    ...
    public void checkConnectState(String targetUserId) {
        String currentUserId = getCurrentUserId();

        if (currentUserId != null) {
            if (!targetUserId.equals(currentUserId)) {
                connecteManager.checkConnectState(currentUserId, targetUserId, connectState -> {
                    ifViewAttached(view -> {
                        view.hideProgress();
                        view.updateConnectButtonState(connectState);
                    });
                });
            } else {
                ifViewAttached(view -> view.updateConnectButtonState(ConnectionState.MY_PROFILE));
            }
        } else {
            ifViewAttached(view -> view.updateConnectButtonState(ConnectionState.NO_ONE_CONNECTED));
        }
    }
    ...
    public void onCollabContainerClick(String postid, View postView) {
