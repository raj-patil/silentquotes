...
const notificationTitle = "SilentQuot";
...
exports.addNewPostToFollowers = functions.database.ref('/posts/{postId}').onCreate((snap, context) => {
    ...
    return getAuthorIdTask.then(authorId  => {
        ...
        const getPostTypeTask = admin.database().ref(`/posts/${postId}/postType`).once('value');

        return getPostTypeTask.then(postType => {
            console.log('post Type ', postType.val());
...
exports.removePostFromBookMark = functions.database.ref('/posts/{postId}').onDelete((snap , context) => {
  const postId = context.params.postId;


  return admin.database().ref().child(bookmarkDbKey).once('value' , function(snapshot) {
        snapshot.forEach(function (childSnapshot) {
            let bookmarkId = childSnapshot.child(postId) ;
            console.log('remove postfrom bokmark list:', bookmarkId, "postId:", postId);
          admin.database().ref().child(bookmarkDbKey).child(childSnapshot.key).child(postId).remove();
          console.log('remove postfrom bokmark list:', bookmarkId, "postId:", postId);
        });
    

   }).catch(fallback => {
       console.error('failer to get bookmark id' , fallback)
   });
